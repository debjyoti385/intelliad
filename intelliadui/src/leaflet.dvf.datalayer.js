	...
	options: {
		...
		onEachRecord: function (layer, record) {
			...
			var photoUrl = '../marker_icon/tweet_back.jpg';
		        var title = "";
                        if (record['photo_title'].length > 1){
                                ...
                                title = "User Information:</br> " + record['owner_name'];
                        }
			...
			layer.on('click', function (e) {
				...
				photo.setAttribute('onload', 'this.style.opacity=0.8;');
				photo.setAttribute('src', photoUrl);
				photo.style.width = width + 'px';

				var photoInfo = L.DomUtil.create('div', 'photo-info', content);
				photoInfo.style.width = (width - 40) + 'px';
		...
		size: 'square',
		attributionText: ' Photos provided by Inmobi are under the copyright of their owners',
